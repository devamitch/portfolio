// ─── FIXED: lastUserMessage extraction ───────────────────────────────────────
// Drop-in replacement for the broken block in your POST handler
// Place this right after `const modelMessages = await convertToModelMessages(messages);`
...
import { z } from "zod";
import { selectRelevantChunks } from "~/lib/amit-context-chunks";
...
//   // 1. New AI SDK UIMessage: parts[] array (preferred path)
...
//   // 2. content is a plain string (older format / some providers)
//   const c = msg?.content;
//   if (typeof c === "string") return c.trim();

//   // 3. content is an array of part objects
//   if (Array.isArray(c)) {
//     return c
...
// //
// // WITH:
// const lastUserMessage = extractTextFromMessage(
//   messages.filter((m: any) => m.role === "user").pop(),
// );

// console.log(`[AURA] Query for context matching: "${lastUserMessage}"`);

// ─── FULL CORRECTED route.ts ──────────────────────────────────────────────────
...
// ─── text extractor (handles all AI SDK message formats) ─────────────────────

function extractTextFromMessage(msg: any): string {
  if (!msg) return "";

  // UIMessage parts[] — newest format
  const parts: any[] = msg?.parts ?? [];
  if (parts.length > 0) {
    const fromParts = parts
      .filter((p: any) => p?.type === "text")
  ...
  // Plain string content — older / server format
  const c = msg?.content;
  if (typeof c === "string") return c.trim();

  // Array of part objects — multipart content
  if (Array.isArray(c)) {
    return c
      .filter((x: any) => x?.type === "text")
      .map((x: any) => String(x?.text ?? ""))
...
export async function POST(req: Request) {
  try {
    ...
    // ── FIXED: proper text extraction before keyword matching ────────────────
    const lastUserMessage = extractTextFromMessage(
      messages.filter((m: any) => m.role === "user").pop(),
    );
    ...
    console.log(
      `[AURA] Query: "${lastUserMessage}" → ${Math.ceil(relevantContext.length / 4)} tokens loaded`,
    );
