...
function speakLine(text: string, onEnd?: () => void) {
  ...
  if (voices.length > 0) {
    ...
    // Safety fallback timer if the browser's TTS engine drops the onend event
...
export default function Preloader({ onComplete }: Props) {
  // START IN THE "AUTH" PHASE TO TRICK THE USER INTO CLICKING
  const [phase, setPhase] = useState<
    "auth" | "boot" | "search" | "reveal" | "exit"
  >("auth");

  // Auth state for the "Hold to hack" trick
  ...
  // ── THE AUDIO UNLOCK HACK ──
  // This runs instantly on mousedown/touchstart, unlocking the browser engine invisibly.
  ...
  // Auth/Hack Interaction logic
  useEffect(() => {
    let authInterval: NodeJS.Timeout;
    if (isHolding && phase === "auth") {
      authInterval = setInterval(() => {
        setAuthProgress((prev) => {
          if (prev >= 100) {
            clearInterval(authInterval);
            setIsHolding(false);
            setPhase("boot"); // Trigger the real sequence
            return 100;
          }
          return prev + 2; // Fills up over ~800ms
        });
      }, 16);
    } else if (!isHolding && phase === "auth") {
      setAuthProgress(0); // Instantly resets if they let go too early
    }
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      // Makes the center pulse faster if they are holding it down
      const pulseSpeed = isHolding ? 0.2 : 0.05;
      ...
      const g = ctx.createRadialGradient(
        ...
        cx,
        cy,
        30 + pulse * (isHolding ? 20 : 10),
      );
      g.addColorStop(
        0,
        `rgba(201,168,76,${(isHolding ? 0.6 : 0.4) + pulse * 0.2})`,
      );
      g.addColorStop(1, "rgba(201,168,76,0)");
      ctx.beginPath();
      ctx.arc(cx, cy, 30 + pulse * (isHolding ? 20 : 10), 0, Math.PI * 2);
      ctx.fillStyle = g;
      ctx.fill();
      t++;
  ...
  return (
    <div
      style={{
        ...
        overflow: "hidden",
        userSelect: "none", // Prevent accidental text highlighting while holding
      ...
      >
        <span
          style={{
            ...
            height: 6,
            borderRadius: "50%",
            background: phase === "auth" ? "#EF4444" : "#C9A84C",
        ...
        <span
          style={{
            fontSize: 8,
            color: phase === "auth" ? "#EF4444" : "rgba(201,168,76,0.65)",
            ...
            <div style={{ textAlign: "center" }}>
              <div
                style={{
                  ...
                  letterSpacing: "0.4em",
                  marginBottom: 8,
                  animation: "plBlink 1.5s infinite",
                }}
              >
                UNAUTHORIZED ACCESS
              </div>
              ...
              >
                MAINFRAME ENCRYPTED. OVERRIDE REQUIRED.
              </div>
            </div>

            {/* THE "BUTTON" - Holding this unlocks the audio */}
            <div
              onMouseDown={() => {
                unlockAudioEngine();
                setIsHolding(true);
              ...
              style={{
                ...
                height: 140,
                borderRadius: "50%",
                border: `1px solid ${isHolding ? "#C9A84C" : "rgba(201,168,76,0.3)"}`,
                background: isHolding ? "rgba(201,168,76,0.08)" : "transparent",
                display: "flex",
                alignItems: "center",
                justifyContent: "center",
                cursor: "pointer",
                ...
                boxShadow: isHolding ? "0 0 40px rgba(201,168,76,0.2)" : "none",
            ...
            >
              ...
              >
                ...
                <circle
                  ...
                  fill="none"
                  strokeDasharray="440"
                  strokeDashoffset={440 - (authProgress / 100) * 440}
              ...
              <div
                style={{
                  fontSize: 10,
                  color: isHolding ? "#C9A84C" : "rgba(201,168,76,0.6)",
                  letterSpacing: "0.2em",
                  textAlign: "center",
                  pointerEvents: "none",
                }}
              >
                {isHolding ? `${authProgress}%` : "HOLD"}
              </div>
            </div>
